...
- Name: booking-flow.spec.ts >> my-booking submit button is disabled when input is empty
- Location: e2e\booking-flow.spec.ts:106:5
...
Locator: getByRole('button', { name: /look up|find|search/i }).first()
Expected: disabled
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /look up|find|search/i }).first()
