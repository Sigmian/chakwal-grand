    ...
    <section className="py-24 bg-surface-base" id="gallery">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div
            className="col-span-2 lg:col-span-2 row-span-2 relative h-72 sm:h-96 lg:h-[480px] rounded-2xl overflow-hidden cursor-pointer group"
            onClick={() => setActive(0)}
          >
            <Image
              src={GALLERY[0].src}
          ...
          {GALLERY.slice(1).map((img, i) => (
            <div
              key={img.src}
              className="relative h-36 sm:h-44 lg:h-[232px] rounded-2xl overflow-hidden cursor-pointer group"
            ...
            >
              ...
              <div className="absolute bottom-3 left-3 right-3 translate-y-3 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <span className="text-[9px] font-bold uppercase tracking-widest text-gold-400 bg-gold-500/20 border border-gold-500/30 px-2 py-0.5 rounded-full">
                  {img.tag}
                </span>
